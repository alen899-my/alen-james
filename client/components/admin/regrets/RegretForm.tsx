...
import { createRegretAction, updateRegretAction } from '@/lib/admin/actions/regrets.actions';
import { Regret } from '@/lib/admin/models/regrets.model';
import {
  Upload, X, Plus, Loader2, Archive, Paperclip, ChevronRight,
  Shield, Film
} from 'lucide-react';
...
export default function RegretForm({ regret }: RegretFormProps) {
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const [images, setImages] = useState<string[]>(regret?.images || []);
  const [videos, setVideos] = useState<string[]>(regret?.videos || []);
  const [isPublic, setIsPublic] = useState<boolean>(regret?.is_public ?? false);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = regret
      ? await updateRegretAction(regret.id, formData)
      : await createRegretAction(null, formData);
      
    setIsSubmitting(false);
    
    if (res.success) { 
      router.push('/admin/regrets'); 
      router.refresh(); 
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 max-w-4xl">
      ...
      <SectionCard
        ...
        title="Regret Details"
        subtitle="Document this moment"
      >
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Field label="Title / Incident" required>
              <input
                type="text"
                name="title"
                defaultValue={regret?.title || ''}
                required
                placeholder="e.g. A missed opportunity"
                className={inputCls}
              />
            </Field>
            ...
            <Field label="Date / Period">
              <input
                type="text"
                name="date"
                defaultValue={regret?.date || ''}
                placeholder="e.g. 2021, or a specific date"
                className={inputCls}
              />
            </Field>
          </div>
          ...
          <Field label="Description / Story">
            <textarea
              name="description"
              defaultValue={regret?.description || ''}
              rows={8}
        ...
        <div className="space-y-6">
          <div className="flex items-start gap-4 p-4 rounded-xl border border-[#e8e2d5] bg-[#faf7f0]">
            ...
            <div>
              ...
              <p className="text-xs text-[#8b9aaa] mt-1">
                If checked, anyone visiting the website can read this entry. If unchecked, it remains secret.
              </p>
            </div>
          </div>
          ...
          <Field label="Individual Password Protection (Optional)">
            <input
              type="text"
              name="password"
              defaultValue={regret?.password || ''}
              placeholder="Leave blank for no password"
              className={inputCls}
            />
            <p className="text-[11px] text-[#aab4be] mt-1.5">
              If you set a password here, users will need to enter it to view this specific entry, even if it's public.
            </p>
          </Field>
        </div>
      </SectionCard>
      ...
      <div className="flex items-center justify-end gap-3 pt-1 pb-8">
        ...
        >
          {isSubmitting
            ...
            : <>{regret ? 'Update Entry' : 'Save Entry'} <ChevronRight size={15} /></>
          }
        </button>
      </div>
    </form>
