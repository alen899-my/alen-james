...
import { createMediaAction, updateMediaAction } from '@/lib/admin/actions/media.actions';
import { MediaGallery } from '@/lib/admin/models/media.model';
import {
  Upload, X, Plus, Loader2, Image as ImageIcon, Paperclip, ChevronRight,
  Film
} from 'lucide-react';
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 max-w-4xl">
      ...
      <SectionCard
        ...
        subtitle="Give your media collection a name and date"
      >
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Field label="Media Name / Title" required>
              <input
                type="text"
                name="title"
                defaultValue={media?.title || ''}
                required
                placeholder="e.g. Summer Vacation, Photoshoot 2024"
                className={inputCls}
              />
            </Field>

            <Field label="Captured / Uploaded Date">
              <input
                type="text"
                name="media_date"
                defaultValue={media?.media_date || ''}
                placeholder="e.g. July 2024, or a specific date"
                className={inputCls}
              />
            </Field>
          </div>
          ...
          <Field label="Short Description">
            <textarea
              ...
              rows={4}
              placeholder="Tell a brief story about this media..."
      ...
      <SectionCard
        ...
        subtitle="Upload your photos and videos"
      >
      ...
      <div className="flex items-center justify-end gap-3 pt-1 pb-8">
        ...
        >
          {isSubmitting
            ...
            : <>{media ? 'Update Media' : 'Save Media'} <ChevronRight size={15} /></>
          }
        </button>
      </div>
    </form>
