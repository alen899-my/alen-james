...
import { createDiaryAction, updateDiaryAction } from '@/lib/admin/actions/diaries.actions';
import { Diary } from '@/lib/admin/models/diaries.model';
import {
  Upload, X, Plus, Loader2, FileText, Paperclip, ChevronRight,
  Shield, Calendar
} from 'lucide-react';
...
export default function DiaryForm({ diary }: DiaryFormProps) {
  ...
  const defaultDate = diary?.incident_date 
    ? new Date(diary.incident_date).toISOString().slice(0, 16) 
    : '';
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 max-w-4xl">
      ...
      <SectionCard
        ...
        title="Diary Entry"
        subtitle="Write down your incident or story"
      >
        <div className="space-y-4">
          <Field label="Title" required>
            <input
              ...
              required
              placeholder="e.g. A beautiful day in Paris..."
              className={inputCls}
            />
          </Field>
          ...
          <Field label="Full Story / Content">
            <textarea
              name="content"
      ...
      <SectionCard
        ...
        subtitle="Control who can see this diary entry"
      >
          ...
          <div className="flex items-start gap-4 p-4 rounded-xl border border-[#e8e2d5] bg-[#faf7f0]">
            ...
            <div>
              ...
              <p className="text-xs text-[#8b9aaa] mt-1">
                If checked, anyone visiting the website can read this diary entry. If unchecked, it remains secret.
              </p>
            </div>
          </div>
      ...
      >
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          ...
          >
            {uploadingMedia ? <Loader2 size={18} className="animate-spin" /> : <Plus size={18} />}
      ...
      <div className="flex items-center justify-end gap-3 pt-1 pb-8">
        ...
        >
          {isSubmitting
            ...
            : <>{diary ? 'Update Diary' : 'Save Diary'} <ChevronRight size={15} /></>
          }
        </button>
      </div>
    </form>
