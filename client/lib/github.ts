...
export async function getContributionCalendars(username?: string) {
    const user = username ?? GITHUB_USERNAME;
    const years = await getContributionYears(user);
    if (years.length === 0) return [];

    const calendars = await Promise.all(
        years.map((year) => getContributionCalendar(user, year)),
    );
